...
#else
#define CPP_TEST_FAIL(function, retval) { fprintf(stderr,"%s:%d:%s:%d:%s:%s\n",__FILE__,__LINE__,function,retval,PAPI_strerror(retval),"$Id: derived.c,v 1.22 2009/09/10 20:19:47 terpstra Exp $\n"); test_fail(__FILE__, __LINE__, function, retval); }
#define CPP_TEST_PASS() { fprintf(stderr,"$Id: derived.c,v 1.22 2009/09/10 20:19:47 terpstra Exp $\n%s:\tPASSED\n",__FILE__); exit(0); }
#define CPP_TEST_SKIP() { fprintf(stderr,"$Id: derived.c,v 1.22 2009/09/10 20:19:47 terpstra Exp $\n%s:\tSKIPPED\n",__FILE__); exit(0); }
#endif
